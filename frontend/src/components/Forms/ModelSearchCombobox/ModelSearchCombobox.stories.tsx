...
import { ModelSearchCombobox } from './ModelSearchCombobox';
  ...
  decorators: [
    Story => (
      <QueryClientProvider client={queryClient}>
        <div className="min-h-96 w-full bg-background p-8">
          <Story />
        </div>
      </QueryClientProvider>
    ),
...
export const Responsive: Story = {
  render: () => (
    <div className="space-y-8">
      <div>
        <h3 className="mb-4 text-sm/6 font-semibold text-foreground">Desktop (Full)</h3>
        <div className="hidden md:block">
          <ModelSearchCombobox variant="full" />
        </div>
      </div>
      <div>
        <h3 className="mb-4 text-sm/6 font-semibold text-foreground">Mobile (Icon)</h3>
