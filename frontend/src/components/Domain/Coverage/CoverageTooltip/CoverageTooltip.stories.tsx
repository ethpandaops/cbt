...
const meta = {
  ...
  decorators: [
    (Story, context) => {
      ...
      return (
        <div ref={containerRef} className="relative min-h-screen bg-background p-8">
          <div className="space-y-4">
            {models.map((model: IncrementalModelItem) => (
              <div key={model.id} data-model-id={model.id} className="rounded-lg border border-border bg-surface p-4">
                <div className="mb-2 font-mono text-sm text-foreground">{model.id}</div>
                <div style={{ height: '24px' }} className="rounded bg-secondary" />
              </div>
            ))}
          </div>
