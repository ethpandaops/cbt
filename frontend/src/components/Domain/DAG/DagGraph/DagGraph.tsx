        ...
        model.depends_on.forEach(dep => {
          if (typeof dep === 'string') {
            ...
            edges.push({
              ...
              type: 'smoothstep',
              animated: true,
              style: { stroke: 'var(--color-incremental)', strokeWidth: 2 },
              markerEnd: {
                type: 'arrowclosed' as const,
                color: 'var(--color-incremental)',
              },
            });
          } else {
            // OR group dependency
            dep.forEach(orDep => {
              edges.push({
                ...
                style: { stroke: 'var(--color-or-edge)', strokeWidth: 2, strokeDasharray: '5,5' },
                markerEnd: {
                  type: 'arrowclosed' as const,
                  color: 'var(--color-or-edge)',
                },
              });
            });
          }
    ...
    <div className={`flex flex-col gap-4 ${className}`}>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 rounded-xl bg-linear-to-r from-surface/95 via-surface/88 to-secondary/45 px-3 py-2 text-xs font-semibold text-primary shadow-sm ring-1 ring-border/70 backdrop-blur-sm">
          ...
          <span className="text-muted">·</span>
          <span className="text-scheduled">{stats.scheduled} sched</span>
          <span className="text-muted">·</span>
          <span className="text-incremental">{stats.incremental} incr</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden rounded-2xl border border-incremental/50 bg-linear-to-br from-background/96 via-background/90 to-secondary/30 shadow-xl ring-1 ring-border/70 backdrop-blur-sm">
        ...
        >
          ...
          <MiniMap
            nodeColor={node => {
              if (node.type === 'external') return 'var(--color-external)';
              if (node.type === 'scheduled') return 'var(--color-scheduled)';
              return 'var(--color-incremental)';
            }}
          />
        </ReactFlow>
      </div>
