...
function DagNodeContent({
  ...
  const borderClass = intervalType
    ? intervalType === 'slot'
      ? 'border-l-4 border-l-interval-slot'
      : intervalType === 'epoch'
        ? 'border-l-4 border-l-interval-epoch'
        : 'border-l-4 border-l-interval-other'
    : '';
  ...
  return (
    <Link to="/model/$id" params={{ id: encodeURIComponent(label) }} className={className}>
      ...
      <div className="flex items-center gap-1.5">
        <div className={`size-1.5 shrink-0 rounded-full ${config.dotColor}`} />
        <div className={`text-[10px] font-bold ${config.labelColor}`}>{config.label}</div>
      </div>

      {/* Model ID - smaller font */}
      <div className="mt-1 truncate font-mono text-[11px] leading-tight font-semibold text-foreground" title={label}>
        {label}
      </div>
        ...
        <div className="mt-1.5 flex flex-col gap-1">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {isIncremental && hasBackfill && (
              <div className="flex items-center gap-0.5 text-[9px] text-incremental" title="Backfill enabled">
            ...
            {contentType && (
              <div className="flex items-center gap-0.5 text-[9px] text-muted" title={`Type: ${contentType}`}>
                {contentType === 'sql' ? (
                  <>
          ...
          {isScheduled && schedule && (
            <div className="flex flex-col gap-0.5">
              <div className="flex items-center gap-0.5 text-[9px] text-scheduled" title={schedule}>
                <ClockIcon className="size-2.5" />
                <span className="truncate">{schedule}</span>
              </div>
              {nextRunText && (
                <div className="text-[8px] font-medium text-scheduled" title="Next scheduled run">
                  Next: {nextRunText}
                </div>
              )}
            </div>
      ...
      <div className={`absolute -inset-[1px] rounded-md transition-all ${config.hoverOverlay}`} />
    </Link>
  );
}
