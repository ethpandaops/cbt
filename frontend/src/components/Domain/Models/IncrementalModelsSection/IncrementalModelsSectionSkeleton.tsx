  ...
  return (
    <div className="space-y-6">
      {[1, 2].map(groupIndex => (
        <div
          key={groupIndex}
          className="overflow-hidden rounded-2xl border border-incremental/45 bg-linear-to-br from-surface/95 via-surface/86 to-secondary/35 p-4 shadow-lg ring-1 ring-border/55 backdrop-blur-sm sm:p-6"
        >
          <div className="relative">
            ...
            <div className="space-y-1.5">
              {[1, 2, 3, 4, 5].map(rowIndex => (
                <div
                  key={rowIndex}
                  className="flex items-center gap-2 rounded-xs bg-secondary/70 px-2 py-1 ring-1 ring-border/35"
                >
                  {/* Model name */}
