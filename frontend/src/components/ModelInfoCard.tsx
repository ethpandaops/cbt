...
export interface InfoField {
  ...
  highlightColor?: 'green' | 'indigo' | 'emerald' | 'amber' | 'blue';
}
...
export function ModelInfoCard({
  ...
  const getFieldClassName = (field: InfoField): string => {
    if (field.variant === 'highlight') {
      ...
      if (color === 'blue') {
        return 'rounded-lg bg-blue-500/10 p-4 ring-1 ring-blue-500/50';
      }
  ...
  const getLabelClassName = (field: InfoField): string => {
    if (field.variant === 'highlight') {
      ...
      if (color === 'blue') {
        return 'mb-1 font-semibold text-blue-400';
      }
  ...
  const getValueClassName = (field: InfoField): string => {
    if (field.variant === 'highlight') {
      ...
      if (color === 'blue') {
        return 'font-mono text-base font-bold text-blue-200';
      }
