...
export function ScheduledModelCard({ id, lastRun, showLink = true }: ScheduledModelCardProps): JSX.Element {
    ...
    <>
      ...
      <div className="relative">
        <div className="mb-2 flex items-start justify-between gap-2 sm:mb-3">
          <h3 className="line-clamp-2 font-mono text-xs font-bold leading-tight text-slate-100 sm:text-sm" title={id}>
            {id}
          </h3>
  ...
  const className =
    'group relative overflow-hidden rounded-2xl border border-emerald-500/30 bg-slate-800/80 p-4 shadow-sm ring-1 ring-slate-700/50 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-emerald-500/20 hover:ring-emerald-500/50 sm:p-5';
