...
// a typed transformation override.
type overrideApplier interface {
	ApplyOverrides(override *models.TransformationOverride)
}

// OverrideReader is the narrow view of the admin service the Applier needs:
// reading the override version counter for cheap change detection and fetching
// all stored overrides. admin.Service satisfies this directly.
type OverrideReader interface {
	GetConfigOverrideVersion(ctx context.Context) (int64, error)
	GetAllConfigOverrides(ctx context.Context) ([]admin.ConfigOverride, error)
}
...
func (l *Applier) applySingleOverride(ov *admin.ConfigOverride) {
	switch ov.Type {
	case string(models.TypeTransformation):
		l.applyTransformationOverride(ov)
	case string(models.TypeExternal):
