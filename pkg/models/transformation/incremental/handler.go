...
func NewHandler(data []byte, adminTable transformation.AdminTable) (*Handler, error) {
	...
	return newHandler(config, adminTable), nil
}

// newHandler wires a Handler and its dependency base to the given config.
func newHandler(config *Config, adminTable transformation.AdminTable) *Handler {
	return &Handler{
...
func (h *Handler) ApplyOverrides(override *transformation.Override) {
	...
	if h.config.Interval != nil {
		h.config.Interval.Min, h.config.Interval.Max, _ = transformation.ApplyIntervalOverride(
			override.Interval, h.config.Interval.Min, h.config.Interval.Max,
		)
	}
	...
	if h.config.Schedules != nil {
		h.config.Schedules.ForwardFill, h.config.Schedules.Backfill = transformation.ApplySchedulesOverride(
			h.config.Schedules.ForwardFill, h.config.Schedules.Backfill, override.Schedules,
		)
	}

	// Apply limits override (creates LimitsConfig if needed)
	if minVal, maxVal, found := transformation.ApplyMinMaxOverride(override.Limits, 0, 0); found {
		if h.config.Limits == nil {
