...
// ErrInvalid is returned when a model ID is not in the expected format.
var ErrInvalid = errors.New("invalid model ID format, expected database.table")
...
func Parse(modelID string) (database, table string, err error) {
	...
	if len(parts) != 2 {
		return "", "", fmt.Errorf("%w: %s", ErrInvalid, modelID)
	}
