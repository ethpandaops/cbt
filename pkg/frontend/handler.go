...
// staticFS is the embedded frontend filesystem. It is a package variable so
// tests can substitute an alternate filesystem.
//
//nolint:gochecknoglobals // Overridable seam for testing the embedded FS.
var staticFS fs.FS = static.FS

// marshalConfig serializes the injected config. It is a package variable so
// tests can exercise the marshal error path.
//
//nolint:gochecknoglobals // Overridable seam for testing the marshal error path.
...
func NewHandler(cfg *InjectedConfig) (http.Handler, error) {
	frontendFS, err := fs.Sub(staticFS, "build/frontend")
	if err != nil {
